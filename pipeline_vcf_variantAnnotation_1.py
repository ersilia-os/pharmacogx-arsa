...
# Inputs:
parser = argparse.ArgumentParser(prog='pipeline_vcf_variantAnnotation',
                                 description='''Pipeline to run variant calling in target NGS''')
...
db1kGPhg38 = "/slgpfs/projects/cli79/reference_data_and_pipelines/data/snpEff/db/GRCh38/1kGPhg38/ERZ822766_merged.vcf.gz"
projectName = options.project

# Create output directory
aName = "_"+options.analysisName if options.analysisName != "" else ""
#outDir = options.workDir+"/"+date_str + "_pipeline_bam_targetVariantCallingRed"+aName
outDir = options.workDir+'/20231129_210444_pipeline_bam_targetVariantCallingRed_coding_1kGPhg38_test2'   # eliminar després
#os.makedirs(outDir)
...
O.write("#!/bin/bash\n")
O.write("#SBATCH --job-name=variantAnnot\n")
O.write("#SBATCH -D '.'\n")
...
O.write("python "+options.pathToReferencePipelines+"/pipeline_vcf_variantAnnotation_3.py -p " +outDir+"/snpeff/"+" -pr "+options.project+" -n "+aName+"\n\n")
