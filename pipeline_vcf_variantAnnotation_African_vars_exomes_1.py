...
# Inputs:
parser = argparse.ArgumentParser(prog='pipeline_vcf_variantAnnotation_African_vars_exomes_1',
                                 description='''Pipeline to perform variant annotation''')
...
dbgnomAD_exomes = options.pathToReferenceData + "/data/snpEff/db/GRCh38/gnomad/release/4.0/vcf/exomes/merged_gnomad.exomes.v4.0.vcf.bgz"

projectName = options.project

# Create output directory
aName = "_"+options.analysisName if options.analysisName != "" else ""
outDir = options.workDir+"/"+date_str + "_pipeline_vcf_variantAnnotation_African_vars_exomes"+aName
os.makedirs(outDir)
...
O.write("#!/bin/bash\n")
O.write("#SBATCH --job-name=ex_variantAnnot\n")
O.write("#SBATCH -D '.'\n")
...
O.write("python "+options.pathToReferencePipelines+"/pipeline_vcf_variantAnnotation_African_vars_exomes_3.py -p " +outDir+"/snpeff/"+" -pr "+options.project+" -n "+aName+"\n\n")
O.close()
