...
# Inputs:
parser  = argparse.ArgumentParser(prog='pipeline_vcf_variantAnnotation_African_vars', description='''Pipeline to run variant annotation''')
...
parser.add_argument('-dbgnomADexDatabase', '--dbgnomAD_exomes',
					dest = "dbgnomAD_exomes",
					action = "store",
					help = "Path to gnomAD exomes database")
...
dbgnomAD_exomes = options.dbgnomAD_exomes
...
# Extract fields for exomes
tsvAnnot_tmp = vcfAnnot.replace(".vcf", "_all_annotations.tsv")
tsvAnnot = vcfAnnot.replace(".vcf", ".tsv")

bashArguments = "cat "+vcfAnnot+" | "+("" if transcriptsToUse == "canonical" else "/apps/SNPEFF/5.1/scripts/vcfEffOnePerLine.pl | ")+"snpsift extractFields -s ',' -e '.' - '"+fieldsToExtract_1+" "+fieldsToExtract_2+" "+fields_1kGPhg38+" "+fields_gnomAD_exomes+" "+fieldsToExtract_3+"' > "+tsvAnnot_tmp
...
for iLine in I:
	if iLine.startswith("CHROM"): 
		...
		for i in range(29,57):
			list[i] = "_".join(['gnomAD_exomes', list[i]])
